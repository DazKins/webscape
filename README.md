...
| Server → client events | Notify eligible clients that something happened once | `chatMessage`, `combatResolved`, `woodcuttingSwing` | Stateless and not replayed; the client owns presentation and expiry |
...
One-shot occurrences use the server's typed domain-event dispatcher instead. Registered subscribers can independently advance quests or project a safe event DTO to interested clients. Client events are sent after that tick's state deltas, so rendering reacts to the latest authoritative state. Chat bubbles, hit splats, and action animations are therefore client-owned effects rather than temporary server entities with TTL components.
