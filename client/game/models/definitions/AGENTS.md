...
- Iterate with `pnpm run build`, then capture with `pnpm run model:screenshot -- --model human --animation run --phase 0.25`. Inspect `.model-previews/`, adjust the definition or sampler, rebuild, and recapture. Use `--all` for the registry gallery and never commit generated previews.
