...
import { sampleTerrainHeight, TERRAIN_SEGMENTS, type TerrainHeightGrid } from "./terrainHeight";
...
export function createTerrainDetails(grid: TerrainHeightGrid, terrain: string[], originX: number, originY: number, blend?: TerrainBlend) {
  const positions: number[] = [], colors: number[] = [];
  const color = new THREE.Color();
  const vertex = (x: number, y: number, z: number) => {
    positions.push(x, y, z); colors.push(color.r, color.g, color.b);
  };
  // Match the rendered terrain triangles, including on non-planar slopes.
  const height = (x: number, z: number) => {
    const left = Math.floor(x * TERRAIN_SEGMENTS) / TERRAIN_SEGMENTS, top = Math.floor(z * TERRAIN_SEGMENTS) / TERRAIN_SEGMENTS;
    const u = (x - left) * TERRAIN_SEGMENTS, v = (z - top) * TERRAIN_SEGMENTS;
    const a = sampleTerrainHeight(grid, left, top), b = sampleTerrainHeight(grid, left + 1 / TERRAIN_SEGMENTS, top);
    const c = sampleTerrainHeight(grid, left, top + 1 / TERRAIN_SEGMENTS), d = sampleTerrainHeight(grid, left + 1 / TERRAIN_SEGMENTS, top + 1 / TERRAIN_SEGMENTS);
  ...
  for (let y = 0; y < grid.sizeY; y++) for (let x = 0; x < grid.sizeX; x++) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const coverage = appearance.detail === "grass" ? blend?.(cx, cz).grass ?? 1 : 1;
      const taper = THREE.MathUtils.smoothstep(coverage, 0.5, 1);
      const size = (appearance.height ?? 0) * (0.65 + random() * 0.7) * taper;
      const rotation = random() * Math.PI * 2;
      color.set(appearance.color).multiplyScalar(0.75 + random() * 0.35);
      // Consume the same random stream even when a shoreline hides this tuft.
      const bladeScales = appearance.detail === "grass" ? Array.from({ length: 3 }, () => 0.7 + random() * 0.3) : [];
      ...
      if (appearance.detail === "grass") {
        for (let blade = 0; blade < 3; blade++) {
          ...
          vertex(cx + dx, height(cx + dx, cz + dz), cz + dz);
          vertex(cx + dx * 1.4, height(cx, cz) + size * bladeScales[blade], cz + dz * 1.4);
        }
      } else {
