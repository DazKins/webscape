...
The server runs systems on a 500 ms tick and then synchronizes each client's interest set in `Game.syncClient`. `world` supplies static registries, world metadata, and day-cycle settings; `chunkUpdate` loads and unloads terrain for that client's current interest; and `gameUpdate` carries the authoritative `serverTick` and per-client ECS component deltas every tick. Only `SerializeableComponent` values visible to that client are considered; unchanged values are omitted, removed components use `data: null`, and `entities: []` is sent when there is no delta. These messages are per-client even when several clients happen to receive equivalent data. Global game time and day/night derive exclusively from the stored tick count: 1,200 ticks of day followed by 1,200 ticks of night. Client presentation may interpolate within a tick but must not advance the cycle beyond the next unconfirmed tick.
